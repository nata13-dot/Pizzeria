...
function PizzeriaApp() {
  ...
  const logout = async () => { try { await api("/logout", session.token, { method: "POST" }); } finally { screenHistory.current=[];await clearApiCache(session.token);await clearSession();setSession(null); } };
  return (
...
function ComboManager({ data, token }: { data: any[]; token: string }) {
  const variants = data.flatMap((product:any)=>(product.variants??[]).map((variant:any)=>({...variant,product})));
  const [kind,setKind]=useState<"package"|"promotion">("package"); const [name,setName]=useState(""); const [price,setPrice]=useState(""); const [selectedVariantId,setSelectedVariantId]=useState<number|null>(null); const [quantity,setQuantity]=useState("1"); const [items,setItems]=useState<any[]>([]); const [combos,setCombos]=useState<any[]>([]); const [busy,setBusy]=useState(false); const [message,setMessage]=useState("");
  const load=()=>api<any[]>("/combos",token).then(setCombos).catch((error)=>setMessage((error as Error).message));
  useEffect(()=>{load();},[token]);
  useEffect(()=>{if(!selectedVariantId&&variants[0])setSelectedVariantId(variants[0].id);},[data]);
  function addItem(){const variant=variants.find((entry:any)=>entry.id===selectedVariantId);const parsed=Number(quantity);if(!variant||parsed<=0)return;const next={product_variant_id:variant.id,quantity:parsed,flavor_required:Boolean(variant.product.flavors?.length),options:[],label:`${variant.product.name} · ${variant.name}`};setItems((current)=>kind==="promotion"?[next]:[...current,next]);setQuantity(kind==="promotion"?"2":"1");}
  async function save(){if(!name.trim()||Number(price)<0||!items.length)return;setBusy(true);setMessage("");try{const payloadItems=items.flatMap(({label:_label,...item})=>kind==="package"&&Number.isInteger(Number(item.quantity))&&Number(item.quantity)>1?Array.from({length:Number(item.quantity)},()=>({...item,quantity:1})):item);await api("/combos",token,{method:"POST",body:JSON.stringify({name:name.trim(),price:Number(price),items:payloadItems})});setName("");setPrice("");setItems([]);setMessage(kind==="promotion"?"Promoción creada.":"Paquete creado.");await load();}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}
  async function remove(combo:any){if(!(await confirmAction(`¿Eliminar ${combo.name}?`)))return;setBusy(true);setMessage("");try{await api(`/combos/${combo.id}`,token,{method:"DELETE"});setMessage("Paquete o promoción eliminado.");await load();}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}
  return <View style={s.formCard}><Text style={s.sectionTitle}>Paquetes y promociones</Text><Text style={s.formIntro}>Combina pizza, conos, alitas u otros productos. En Caja se podrá elegir el sabor de cada componente que tenga sabores.</Text><View style={s.types}><Pressable onPress={()=>{setKind("package");setItems([]);setQuantity("1");}} style={[s.type,kind==="package"&&s.typeActive]}><Text>Paquete de productos</Text></Pressable><Pressable onPress={()=>{setKind("promotion");setItems([]);setQuantity("2");}} style={[s.type,kind==="promotion"&&s.typeActive]}><Text>Promoción por piezas</Text></Pressable></View><View style={s.inlineFields}><TextInput style={[s.input,s.flexField]} placeholder={kind==="promotion"?"Ej. 2 pizzas medianas":"Nombre del paquete"} value={name} onChangeText={setName}/><TextInput style={[s.input,s.flexField]} placeholder="Precio total" value={price} onChangeText={setPrice} keyboardType="decimal-pad"/></View><Text style={s.label}>{kind==="promotion"?"Producto de la promoción":"Agregar producto al paquete"}</Text><View style={s.types}>{variants.map((variant:any)=><Pressable key={variant.id} onPress={()=>setSelectedVariantId(variant.id)} style={[s.type,selectedVariantId===variant.id&&s.typeActive]}><Text>{variant.product.name} · {variant.name}</Text></Pressable>)}</View><View style={s.inlineFields}><TextInput style={[s.input,s.flexField]} placeholder="Cantidad" value={quantity} onChangeText={setQuantity} keyboardType="decimal-pad"/><Pressable disabled={!selectedVariantId||Number(quantity)<=0} onPress={addItem} style={[s.secondaryButton,(!selectedVariantId||Number(quantity)<=0)&&s.disabled]}><Text style={s.secondaryText}>{kind==="promotion"?"Asignar promoción":"Agregar al paquete"}</Text></Pressable></View>{items.map((item,index)=><View key={`${item.product_variant_id}-${index}`} style={s.recipeRow}><View style={s.rowText}><Text style={s.label}>{item.quantity} × {item.label}</Text><Text style={s.muted}>{item.flavor_required?"El cliente elegirá el sabor en Caja":"Sin selección de sabor"}</Text></View><Pressable onPress={()=>setItems((current)=>current.filter((_row,rowIndex)=>rowIndex!==index))}><Text style={s.remove}>Quitar</Text></Pressable></View>)}<Pressable disabled={busy||!name.trim()||!price||!items.length} onPress={save} style={[s.primary,(busy||!name.trim()||!price||!items.length)&&s.disabled]}><Text style={s.primaryText}>{busy?"Guardando...":kind==="promotion"?"Crear promoción":"Crear paquete"}</Text></Pressable>{!!message&&<Text style={s.notice}>{message}</Text>}{!!combos.length&&<View style={s.formStep}><Text style={s.label}>Paquetes y promociones activos</Text>{combos.map((combo:any)=><View key={combo.id} style={s.recipeRow}><View style={s.rowText}><Text style={s.productName}>{combo.name}</Text><Text style={s.muted}>${Number(combo.price).toFixed(2)} · {combo.items.length===1&&Number(combo.items[0].quantity)>1?"Promoción":"Paquete"}</Text></View><Pressable disabled={busy} onPress={()=>remove(combo)}><Text style={s.remove}>Eliminar</Text></Pressable></View>)}</View>}</View>;
}
function ProductsView({ data, token, onSaved, canConfigure }: { data: any[]; token: string; onSaved: () => void; canConfigure: boolean }) {
  ...
  function editProduct(product:any){setEditingProductId(product.id);setEditDraft({...product,name:product.name??"",description:product.description??"",image_data_uri:product.image_data_uri??"",variants:(product.variants??[]).map((variant:any)=>({...variant,price:String(variant.price),max_flavors:String(variant.max_flavors??1)})),flavors:(product.flavors??[]).map((flavor:any)=>({...flavor})),removedFlavorIds:[],removedVariantIds:[]});setMessage("");}
  function updateEditVariant(id:number, changes:any){setEditDraft((current:any)=>({...current,variants:current.variants.map((variant:any)=>variant.id===id?{...variant,...changes}:variant)}));}
  async function removeEditVariant(variant:any){if(!(await confirmAction(`¿Quitar la variante ${variant.name}? Se eliminará al guardar los cambios.`)))return;setEditDraft((current:any)=>({...current,variants:current.variants.filter((item:any)=>item.id!==variant.id),removedVariantIds:[...current.removedVariantIds,variant.id]}));}
  function updateEditFlavor(id:number|string, value:string){setEditDraft((current:any)=>({...current,flavors:current.flavors.map((flavor:any)=>flavor.id===id?{...flavor,name:value}:flavor)}));}
  function addEditFlavor(){setEditDraft((current:any)=>({...current,flavors:[...current.flavors,{id:`new-${Date.now()}-${current.flavors.length}`,name:""}]}));}
  function removeEditFlavor(flavor:any){setEditDraft((current:any)=>({...current,flavors:current.flavors.filter((item:any)=>item.id!==flavor.id),removedFlavorIds:typeof flavor.id==="number"?[...current.removedFlavorIds,flavor.id]:current.removedFlavorIds}));}
  async function configureStuffedCrust(){const enabledVariants=editDraft.variants.filter((variant:any)=>variant.allows_stuffed_crust);const existingStuffedRules=editDraft.variants.flatMap((variant:any)=>(variant.modifier_rules??[]).filter((rule:any)=>rule.modifier?.type==="stuffed_crust").map((rule:any)=>({variantId:variant.id,modifierId:rule.modifier_id})));if(enabledVariants.length){const modifierOptions=[{name:"Orilla rellena de queso con habanero",flavor:"habanero"},{name:"Orilla rellena de queso con chipotle",flavor:"chipotle"}];const existing=await api<any[]>("/modifiers",token);const modifiers=[];for(const option of modifierOptions){let modifier=existing.find((item:any)=>item.type==="stuffed_crust"&&item.name.toLocaleLowerCase().includes(option.flavor));if(!modifier)modifier=await api<any>("/modifiers",token,{method:"POST",body:JSON.stringify({name:option.name,type:"stuffed_crust",price:20,items:[]})});else if(modifier.name!==option.name||modifier.type!=="stuffed_crust"||Number(modifier.price)!==20)modifier=await api<any>(`/modifiers/${modifier.id}`,token,{method:"PATCH",body:JSON.stringify({name:option.name,type:"stuffed_crust",price:20})});modifiers.push(modifier);}for(const variant of enabledVariants){for(const modifier of modifiers){await api(`/product-variants/${variant.id}/modifiers`,token,{method:"POST",body:JSON.stringify({modifier_id:modifier.id,allowed:true,price_override:20})});}}}for(const rule of existingStuffedRules.filter((rule:any)=>!enabledVariants.some((variant:any)=>variant.id===rule.variantId))){await api(`/product-variants/${rule.variantId}/modifiers/${rule.modifierId}`,token,{method:"DELETE"});}}
  async function saveProductChanges(){if(!editingProductId||!editDraft)return;setBusy(true);setMessage("");try{const productPayload={name:editDraft.name.trim(),description:editDraft.description.trim()||null,image_data_uri:editDraft.image_data_uri||null,type:editDraft.type};const saveProductFields=()=>api(`/products/${editingProductId}`,token,{method:"PATCH",body:JSON.stringify(productPayload)});const saveVariants=async()=>{for(const variant of editDraft.variants){await api(`/product-variants/${variant.id}`,token,{method:"PATCH",body:JSON.stringify({name:variant.name.trim(),price:Number(variant.price),max_flavors:Number(variant.max_flavors),allows_half_and_half:Boolean(variant.allows_half_and_half),allows_stuffed_crust:Boolean(variant.allows_stuffed_crust)})});}};if(editDraft.type==="pizza"){await saveProductFields();await saveVariants();}else{await saveVariants();await saveProductFields();}await configureStuffedCrust();for(const variantId of editDraft.removedVariantIds){await api(`/product-variants/${variantId}`,token,{method:"DELETE"});}for(const flavor of editDraft.flavors){if(typeof flavor.id==="number")await api(`/product-flavors/${flavor.id}`,token,{method:"PATCH",body:JSON.stringify({name:flavor.name.trim()})});else await api(`/products/${editingProductId}/flavors`,token,{method:"POST",body:JSON.stringify({name:flavor.name.trim()})});}for(const flavorId of editDraft.removedFlavorIds){await api(`/product-flavors/${flavorId}`,token,{method:"DELETE"});}setEditingProductId(null);setEditDraft(null);setMessage("Producto actualizado correctamente.");await onSaved();}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}
  async function deleteProduct(product:any){if(!(await confirmAction(`¿Eliminar ${product.name}? Dejará de aparecer en Caja y en el catálogo.`)))return;setBusy(true);setMessage("");try{await api(`/products/${product.id}`,token,{method:"DELETE"});if(editingProductId===product.id){setEditingProductId(null);setEditDraft(null);}setMessage("Producto eliminado.");await onSaved();}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}
      ...
      renderItem={({ item }) => (
        <View style={s.row}>
          ...
          <View style={s.rowText}>
            ...
            {canConfigure&&<View style={s.reportActions}><Pressable disabled={busy} onPress={()=>editProduct(item)}><Text style={s.editLink}>Editar producto</Text></Pressable><Pressable disabled={busy} onPress={()=>deleteProduct(item)}><Text style={s.deleteLink}>Eliminar</Text></Pressable></View>}
            {editingProductId===item.id&&editDraft&&<View style={s.inlineEditor}>
              <TextInput style={s.input} placeholder="Nombre del producto" value={editDraft.name} onChangeText={(value)=>setEditDraft((current:any)=>({...current,name:value}))}/>
              <TextInput style={[s.input,s.multilineInput]} multiline maxLength={5000} placeholder="Descripción corta" value={editDraft.description} onChangeText={(value)=>setEditDraft((current:any)=>({...current,description:value}))}/>
              <LogoPicker value={editDraft.image_data_uri} onChange={(value)=>setEditDraft((current:any)=>({...current,image_data_uri:value}))} onError={setMessage} emptyLabel="Seleccionar o arrastrar foto" changeLabel="Cambiar foto" />
              <Text style={s.label}>Tipo de producto</Text>
              <View style={s.types}>{[["pizza","Pizza"],["wings","Alitas"],["fries","Papas"],["nuggets","Nuggets"],["cone","Cono"],["beverage","Bebida"],["extra","Extra"],["other","Otro"]].map(([key,label])=><Pressable key={key} onPress={()=>setEditDraft((current:any)=>({...current,type:key,variants:key==="pizza"?current.variants:current.variants.map((variant:any)=>({...variant,allows_half_and_half:false,allows_stuffed_crust:false}))}))} style={[s.type,editDraft.type===key&&s.typeActive]}><Text>{label}</Text></Pressable>)}</View>
              {!!editDraft.variants.length&&<Text style={s.label}>Variantes / tamaños</Text>}
              {editDraft.variants.map((variant:any)=><View key={variant.id} style={s.formStep}><View style={s.stepHeading}><Text style={s.label}>{variant.name||"Variante sin nombre"}</Text><Pressable disabled={busy} onPress={()=>removeEditVariant(variant)} style={s.dangerButton}><Text style={s.dangerText}>Eliminar variante</Text></Pressable></View><TextInput style={s.input} placeholder="Variante/tamaño" value={variant.name} onChangeText={(value)=>updateEditVariant(variant.id,{name:value})}/><View style={s.inlineFields}><TextInput style={[s.input,s.flexField]} placeholder="Precio" value={variant.price} onChangeText={(value)=>updateEditVariant(variant.id,{price:value})} keyboardType="decimal-pad"/><TextInput style={[s.input,s.flexField]} placeholder="Máx. sabores" value={variant.max_flavors} onChangeText={(value)=>updateEditVariant(variant.id,{max_flavors:value})} keyboardType="number-pad"/></View>{editDraft.type==="pizza"&&<View style={s.types}><Pressable onPress={()=>updateEditVariant(variant.id,{allows_half_and_half:!variant.allows_half_and_half})} style={[s.type,variant.allows_half_and_half&&s.typeActive]}><Text>{variant.allows_half_and_half?"Mitad y mitad habilitada":"Habilitar mitad y mitad"}</Text></Pressable><Pressable onPress={()=>updateEditVariant(variant.id,{allows_stuffed_crust:!variant.allows_stuffed_crust})} style={[s.type,variant.allows_stuffed_crust&&s.typeActive]}><Text>{variant.allows_stuffed_crust?"Orilla rellena habilitada (+$20)":"Habilitar orilla rellena (+$20)"}</Text></Pressable></View>}</View>)}
              <View style={s.stepHeading}><Text style={s.label}>Sabores</Text><Pressable disabled={busy} onPress={addEditFlavor} style={s.secondaryButton}><Text style={s.secondaryText}>+ Agregar sabor</Text></Pressable></View>
              {!editDraft.flavors.length&&<Text style={s.muted}>Este producto todavía no tiene sabores.</Text>}
              {editDraft.flavors.map((flavor:any)=><View key={flavor.id} style={s.flavorEditRow}><TextInput style={[s.input,s.flexField]} placeholder="Nombre del sabor" value={flavor.name} onChangeText={(value)=>updateEditFlavor(flavor.id,value)}/><Pressable disabled={busy} onPress={()=>removeEditFlavor(flavor)} style={s.dangerButton}><Text style={s.dangerText}>Quitar</Text></Pressable></View>)}
              <View style={s.reportActions}><Pressable disabled={busy||!editDraft.name.trim()||editDraft.flavors.some((flavor:any)=>!flavor.name.trim())||editDraft.variants.some((variant:any)=>!variant.name.trim()||!variant.price||Number(variant.max_flavors)<1||(variant.allows_half_and_half&&Number(variant.max_flavors)<2))} onPress={saveProductChanges} style={[s.smallButton,(busy||!editDraft.name.trim()||editDraft.flavors.some((flavor:any)=>!flavor.name.trim())||editDraft.variants.some((variant:any)=>!variant.name.trim()||!variant.price||Number(variant.max_flavors)<1||(variant.allows_half_and_half&&Number(variant.max_flavors)<2)))&&s.disabled]}><Text style={s.primaryText}>{busy?"Guardando...":"Guardar cambios"}</Text></Pressable>{!!editDraft.image_data_uri&&<Pressable disabled={busy} onPress={()=>setEditDraft((current:any)=>({...current,image_data_uri:""}))} style={s.secondaryButton}><Text style={s.secondaryText}>Quitar imagen</Text></Pressable>}<Pressable disabled={busy} onPress={()=>{setEditingProductId(null);setEditDraft(null);}} style={s.secondaryButton}><Text style={s.secondaryText}>Cancelar</Text></Pressable></View>
            </View>}
          </View>
...
const s = StyleSheet.create({
  ...
  deleteLink: { color: "#a82e20", fontWeight: "800", marginTop: 8 },
  inlineEditor: { gap: 10, marginTop: 14, width: "100%" },
  flavorEditRow: { alignItems: "center", flexDirection: "row", gap: 10 },

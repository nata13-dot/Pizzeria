...
function ComboManager({ data, token }: { data: any[]; token: string }) {
  ...
  async function save(){if(!name.trim()||Number(price)<0||!items.length)return;setBusy(true);setMessage("");try{await api("/combos",token,{method:"POST",body:JSON.stringify({name:name.trim(),price:Number(price),items:items.map(({label:_label,...item})=>item)})});setName("");setPrice("");setItems([]);setMessage(kind==="promotion"?"Promoción creada.":"Paquete creado.");await load();}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}

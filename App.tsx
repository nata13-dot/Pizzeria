...
function ProductsView({ data, token, onSaved, canConfigure }: { data: any[]; token: string; onSaved: () => void; canConfigure: boolean }) {
  ...
  function updateEditFlavor(id:number, value:string){setEditDraft((current:any)=>({...current,flavors:current.flavors.map((flavor:any)=>flavor.id===id?{...flavor,name:value}:flavor)}));}
  async function saveProductChanges(){if(!editingProductId||!editDraft)return;setBusy(true);setMessage("");try{const productPayload={name:editDraft.name.trim(),description:editDraft.description.trim()||null,image_data_uri:editDraft.image_data_uri||null,type:editDraft.type};const saveProductFields=()=>api(`/products/${editingProductId}`,token,{method:"PATCH",body:JSON.stringify(productPayload)});const saveVariants=async()=>{for(const variant of editDraft.variants){await api(`/product-variants/${variant.id}`,token,{method:"PATCH",body:JSON.stringify({name:variant.name.trim(),price:Number(variant.price),max_flavors:Number(variant.max_flavors),allows_half_and_half:Boolean(variant.allows_half_and_half)})});}};if(editDraft.type==="pizza"){await saveProductFields();await saveVariants();}else{await saveVariants();await saveProductFields();}for(const flavor of editDraft.flavors){await api(`/product-flavors/${flavor.id}`,token,{method:"PATCH",body:JSON.stringify({name:flavor.name.trim()})});}setEditingProductId(null);setEditDraft(null);setMessage("Producto actualizado correctamente.");await onSaved();}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}
        ...
        <View style={s.row}>
          ...
          <View style={s.rowText}>
            ...
            {editingProductId===item.id&&editDraft&&<View style={s.inlineEditor}>
              ...
              {!!editDraft.flavors.length&&<Text style={s.label}>Sabores</Text>}
              {editDraft.flavors.map((flavor:any)=><TextInput key={flavor.id} style={s.input} placeholder="Nombre del sabor" value={flavor.name} onChangeText={(value)=>updateEditFlavor(flavor.id,value)}/>)}

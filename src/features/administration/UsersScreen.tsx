...
function CreateUser({ token, roles, onSaved, setMessage }: { token: string; roles: Role[]; onSaved: () => Promise<void>; setMessage: (message: string) => void }) {
  ...
  const canSave = !busy && !!name.trim() && !!email.trim() && passwordLength >= 8 && !!roleId;
  async function save() {
    if (!canSave) {
      setMessage(passwordLength < 8 ? `La contraseña debe tener al menos 8 caracteres. Faltan ${8 - passwordLength}.` : "Completa nombre, correo, contraseña y rol antes de registrar el usuario.");
      return;
    }
    setBusy(true); setMessage("");
  ...
  return <View style={styles.card}><Text style={styles.title}>Registrar usuario</Text><View style={styles.inline}><TextInput style={[styles.input, styles.flex]} value={name} onChangeText={setName} placeholder="Nombre" /><TextInput style={[styles.input, styles.flex]} value={username} onChangeText={setUsername} autoCapitalize="none" placeholder="Usuario opcional" /></View><TextInput style={styles.input} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" placeholder="Correo" /><TextInput style={styles.input} value={password} onChangeText={setPassword} secureTextEntry autoCapitalize="none" placeholder="Contraseña (mínimo 8 caracteres)" /><Text style={passwordLength >= 8 ? styles.passwordValid : styles.passwordHint}>{passwordLength >= 8 ? "✓ Contraseña válida" : `Contraseña: ${passwordLength}/8 caracteres${passwordLength ? ` · faltan ${8 - passwordLength}` : ""}`}</Text><RoleChoices roles={roles} selectedId={roleId} setSelectedId={setRoleId} /><Pressable disabled={!canSave} style={[styles.primary, !canSave && styles.disabled]} onPress={save}><Text style={styles.primaryText}>{busy ? "Guardando..." : "Registrar usuario"}</Text></Pressable></View>;
}
